...
describe("tool card follow scroll", () => {
  ...
  /**
   * A wheel notch inside a live think card must park it on the first notch, even
   * though the next chunk lands before any scroll event is delivered — the case
   * that made scrolling a streaming card feel like it "bounced back".
   */
  it("parks on the first wheel notch and survives the next chunk", () => {
    const box = { scrollHeight: 800, clientHeight: 200 };
    const { container, rerender } = render(<Harness follow text="live" />);
    const el = container.firstElementChild as HTMLElement;
    mockScroller(el, box);
  ...
  it("re-pins once the wheel brings the card back to its last line", () => {
    const box = { scrollHeight: 800, clientHeight: 200 };
    const { container, rerender } = render(<Harness follow text="live" />);
    const el = container.firstElementChild as HTMLElement;
    mockScroller(el, box);
