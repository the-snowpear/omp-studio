...
function snapshot(sessionId: SessionId | undefined, rows: ConversationSnapshot["rows"]): ConversationSnapshot {
  return {
    state: resetConversation(1, sessionId === undefined ? null : { sessionId }, rows.length === 0 ? "idle" : "ready"),
    rows,
    demo: false,
    loadingOlder: false,
