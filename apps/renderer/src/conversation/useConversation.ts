...
export type UseConversationInput = { readonly preview: boolean; readonly client: ConversationClient | null; readonly identity: ConversationIdentity | null; readonly canRead: boolean; readonly runtimeConnected: boolean; readonly previewThreadId?: string };
export type UseConversationResult = ConversationSnapshot & { readonly engine: ConversationEngine; loadOlder: () => void; reload: () => Promise<void>; restoreFromUser: (itemId: string) => boolean; trackPending: (pending: PendingUser) => void; failPending: (requestId: string, error: string) => void; dropPending: (requestId: string) => void };
...
export function useConversation(input: UseConversationInput): UseConversationResult {
  const engine = useMemo(() => createConversationEngine(defaultConversationEngineInput(input)), [input.preview, input.client, input.identity?.sessionId, input.identity?.runtimeEpoch, input.identity?.transcriptRevision, input.canRead, input.runtimeConnected, input.previewThreadId]);
  ...
  callbacks.current = { loadOlder: () => engine.loadOlder(), reload: () => engine.reload(), restoreFromUser: (itemId) => engine.restoreFromUser(itemId), trackPending: (pending) => engine.trackPending(pending), failPending: (requestId, error) => engine.failPending(requestId, error), dropPending: (requestId) => engine.dropPending(requestId) };
  const stable = useRef({
