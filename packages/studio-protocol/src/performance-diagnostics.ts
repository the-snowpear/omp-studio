...
export function createMemorySampleGate(options: MemorySampleGateOptions): MemorySampleGate {
  ...
  return {
    observe(input) {
      ...
      const ratioFor = (field: MemoryField): number => (field === "heapUsedBytes" || field === "heapTotalBytes" ? heapRatio : nativeRatio);
      for (const field of MEMORY_FIELDS) {
        if (significantChange(baseline[field], sample[field], ratioFor(field), minDelta)) return accept(sample, "memory");
      }
...
export function formatMemorySampleLine(sample: MemorySample, options: FormatMemorySampleOptions = {}): string {
  ...
  for (const [key, value] of Object.entries(options.prefix ?? {})) {
    ...
    const text = typeof value === "number" ? (Number.isFinite(value) ? String(value) : undefined) : PREFIX_VALUE.test(value) ? value : undefined;
