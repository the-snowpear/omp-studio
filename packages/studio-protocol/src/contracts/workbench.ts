...
export function validateWorkbenchOperation(value: unknown): asserts value is WorkbenchOperation {
  ...
  if (kind === "accounts.status" && input.refresh !== undefined && typeof input.refresh !== "boolean") throw new Error("Invalid account refresh flag");
...
export function validateWorkbenchResult(kind: WorkbenchOperation["kind"], value: unknown): void {
  ...
  if (kind === "accounts.status") { validateAccountStatus(value); return; }
  if (kind === "services.list") {
    ...
    for (const item of result.models) {
      const row = record(item, ["selector", "name", "provider", "kind", "image", "reasoning", "contextWindow", "maxTokens", "webSearch"]);
