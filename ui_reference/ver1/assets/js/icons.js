/* ============================================================
   OMP Studio — Inline SVG icon set (16×16, stroke)
   用法：静态 HTML 写 <svg class="icon" data-icon="search"></svg>，
   JS 渲染内容用 OMP.icon('search', 'sm')。
   file:// 下无外部请求，双击即可运行。
   ============================================================ */
(function () {
  const P = {
    ...
    wrench: '<path d="M9.8 2.2a3.5 3.5 0 0 0-4.6 4.6L2 10l4 4 3.2-3.2a3.5 3.5 0 0 0 4.6-4.6L11 9 9 7z"/>',
  ...
  window.OMP.icon = function (name, extra) {
    return (
      '<svg class="icon' + (extra ? ' ' + extra : '') + '" viewBox="0 0 16 16" aria-hidden="true">' +
      svgInner(name) +
      '</svg>'
    );
  };
  window.OMP.injectIcons = function (root) {
    (root || document).querySelectorAll('svg[data-icon]').forEach(function (el) {
      el.setAttribute('viewBox', '0 0 16 16');
      el.innerHTML = svgInner(el.getAttribute('data-icon'));
