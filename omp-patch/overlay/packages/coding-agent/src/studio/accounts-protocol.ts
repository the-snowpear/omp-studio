...
export interface StudioAccountStatus {
	...
	label: string;
	organization?: string;
	source: "oauth" | "api-key" | "ambient";
	active: boolean;
	limits: AccountQuotaWindow[];
	reportedAt?: number;
	resets: AccountResetStatus;
}
...
function text(value: unknown): void {
	if (typeof value !== "string" || !value.trim() || value.length > 512 || /[\u0000-\u001f]/u.test(value))
		throw new Error("Invalid account status text");
}
function number(value: unknown): void {
	if (typeof value !== "number" || !Number.isFinite(value) || value < 0)
		throw new Error("Invalid account status number");
}
function optional(row: Record<string, unknown>, keys: string[], validate: (value: unknown) => void): void {
	for (const key of keys) if (row[key] !== undefined) validate(row[key]);
}
function boolean(value: unknown): void {
	if (typeof value !== "boolean") throw new Error("Invalid account status flag");
}
function list(value: unknown, max: number, validate: (value: unknown) => void): void {
	if (!Array.isArray(value) || value.length > max) throw new Error("Invalid account status list");
...
function resets(value: unknown): void {
	...
	if (!["unfetched", "available", "unavailable"].includes(row.state as string))
		throw new Error("Invalid reset status");
	optional(row, ["availableCount", "redeemableCount"], number);
	optional(row, ["eligible"], boolean);
	list(row.credits, 64, value => {
		...
		text(credit.title);
		optional(credit, ["status", "expiresAt"], text);
		optional(credit, ["remainingCount"], number);
		optional(credit, ["usable", "requiresLimit"], boolean);
		list(credit.clears, 32, text);
	});
}
export function validateAccountStatus(value: unknown): asserts value is AccountStatusResult {
	...
	list(result.accounts, 200, value => {
		...
		optional(row, ["organization"], text);
		optional(row, ["reportedAt"], number);
		boolean(row.active);
		if (!["oauth", "api-key", "ambient"].includes(row.source as string)) throw new Error("Invalid account source");
		list(row.limits, 64, quota);
		resets(row.resets);
	});
	list(result.unassigned, 200, value => {
		const row = object(value, ["provider", "reportedAt", "limits"]);
		text(row.provider);
		number(row.reportedAt);
		list(row.limits, 64, quota);
	});
}
