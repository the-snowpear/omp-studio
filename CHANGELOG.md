...
- **一键启动生产版（`启动生产版.cmd`）**：`scripts/preview.mjs` 支持 `OMP_PREVIEW_MODE=production`——改名后的预览 exe 报告 `app.isPackaged === true`，桌面端本就直接加载刚构建的 `apps/renderer/dist`，此前流程里随行启动的 vite dev server 从未被使用，白占一个 node 进程与启动等待；生产模式将其跳过（构建 → preload → Electron 一条龙不变）。默认（dev 模式）行为保持原样，供需要 HMR 调试 renderer 的场景使用。
...
- **网络搜索配置页重做（模型配置页第 4 个 tab）**：删除原前后端实现后按 OMP 上游 `web/search` 能力重建。新面板为「生效链路 + 双分区」布局：顶部状态卡实时推算展示当前实际会用的搜索链（显式 `providers.webSearchOrder` 优先，其后是按内置顺序凭证就绪的自动链），下面分「优先链」（拖拽/箭头排序）与「供应商库」（搜索框 + 全部/已就绪/未配置/免凭证/已排除筛选，每行显示引擎的凭证要求描述与凭证徽标）；tab 计数 chip 从 order 长度改为就绪引擎数，命令面板补上此前缺失的网络搜索入口。功能补全：read model 的每个供应商带上游 `SEARCH_PROVIDER_OPTIONS` 描述；`searxng` 新增 `categories` / `engines`（逗号分隔）、`language`、`safesearch`（0/1/2，空 = 实例默认）四个此前面板未覆盖的字段，SearXNG / Exa 高级区改为默认收起的折叠区。写入仍走单命令 `models.webSearch.set`（Host 直写 `config.yml`，密钥只写不读，`safesearch: null` 删键），入站校验、reducer 敏感命令登记与 testkit fixture 同步；预览模式 demo fixture 带描述与新字段；新增 `WebSearchPanel` 组件测试（8 例）与 adapter 新字段写入/清除测试。
  - **四项交互优化**（同日反馈）：优先链改为与供应商卡片相同的 pointer 拖拽（pointerdown/move/up + translate3d 位移动画与落位过渡，替换掉在 Electron 下失灵的 HTML5 DnD），并保留上移/下移按钮；状态卡去掉标题图标、开关去掉 ON/OFF 胶囊封装直接裸显；供应商库行改为严格网格列（品牌图标 | 名称/ID | 描述 | 凭证徽标 | 居右操作区），23 个引擎接入真实品牌图标（Simple Icons SVG 9 个 + 官网 favicon PNG 8 个 + 既有品牌 5 个 + Public Web 用地球图标，`brands.tsx` 扩展 PNG 支持与搜索引擎 id 映射）；「配置凭证」不再切到供应商页，改为就地弹窗：OAuth 登录/登出走 `models.login.start/logout`（read model 新增每引擎的 `envKeys` 与 `loginId`，由 Host 从凭证表派生），免凭证引擎直接说明，环境变量名可一键复制，预览模式登录只翻转 demo 数据。
  - **凭证直填与弹窗定位**（同日反馈）：弹窗主路径改为「直接粘贴 API 密钥」——新增 `models.webSearch.credential.set` / `credential.remove` 命令，Host 把 `api_key` 凭证按上游凭证库的行格式（`data` = JSON、`identity_key` NULL、复用已禁用行）写进本机 agent.db，运行时 `getApiKey` 原生读取，非必要不再依赖 `omp login` / 环境变量；OAuth 登录与环境变量提示降级为「其他方式」辅助区。密钥的存储 id 与 OAuth 登录 id 分离派生（如 Gemini 的密钥写在 `google`、OAuth 走 `google-gemini-cli`，`WEB_SEARCH_AUTH_IDS` 相应补 `google`），read model 新增每引擎 `apiKeyId` 与 `credentialKind`（api-key / oauth / env，env 优先）供弹窗如实展示现状。弹窗改用 React portal 挂载到 `document.body`（修复被页面过渡祖先 transform 劫持的 `position: fixed`——此前弹窗定位在滚动区内），并整窗居中偏上约 7vh。
  - **OAuth 登录白名单**（同日报错修复）：应用内「登录」按钮只对运行时 OAuth 注册表中真实存在且可无头登录的引擎暴露（perplexity / anthropic / openai-codex / xai / zai / exa / kagi / tavily / kimi-code / parallel / synthetic）。brave、firecrawl、jina、tinyfish 不在注册表（点击必报 Unknown OAuth provider）、google-gemini-cli 的流程先要终端提示（RPC 模式必拒「需要交互式输入」），这些引擎不再显示登录按钮，改由 API 密钥 / 环境变量路径覆盖；Gemini 的 OAuth 行（终端登录产生的）仍会被凭证状态识别。「也可以在终端运行 omp login …」提示行按反馈移除。
...
- **发送消息瞬间「卡一下、上下跳两三下」**：新行（乐观用户行、落盘正式行、assistant 流式行）原按 132px 估高挂载、下一帧才实测修正，贴底跟随因此出现两到三段位移；且发送级联期间 dev 模式 React（`jsxDEV` 开发检查）产生数十个 130-330ms 长任务，把高度修正与跟随滚动推迟 300ms+ 批量补上，位移变成可见跳动。`ConversationVirtualList` 现在在行挂载的 ref 回调里把真实高度同步喂回 `virtualizer.resizeItem`——挂载帧即真实高度，总高度一步到位（实测 dev 下从 3 跳降至跟随延迟致的余跳；生产构建下 CPU profile 证实 `jsxDEV` 占发送后 ~1.2s CPU，生产为 0，发送瞬间长任务 45 个 → 1 个、锚行跳变 0 次）。
...
- **流式期间的每帧「写后读」与 minimap 写入风暴（操作迟滞系列第二刀）**：贴底跟随形成固定循环——React 提交正文（写）→ 贴底写 `scrollTop`（写）→ scroll 事件里 minimap `syncViewport` 全量写 aria/style（写）+ `useConversationScroll.onScroll` 读 `scrollHeight`（写在后=强制同步布局）。实测流式峰值 RecalcStyle/Layout 仍达 ~200-310 次/秒（每帧 3-5 轮），点击/拖动要排队补布局才响应；Chromium 原生分配高水位随 churn 持续上涨（JS 堆全程有界，GB 级全在原生侧）。三项修复：① `syncViewport` 写入值缓存，相同值零写入；② 流式期间（`busy`）minimap 滚动同步限频到 ~10Hz（尾随定时器保证最终位置落地，滑块位移 10Hz 肉眼无感）；③ `useConversationScroll` 缓存 scroller 几何（由贴底 RO 顺带刷新），`onScroll` 的距底计算不再读布局属性。修复后每帧写入方从 4-5 个收敛到 React 提交本身。
- **长流式会话内存狂涨（单会话 2.8GB）与整应用操作迟滞**：根因是 `ConversationVirtualList` 的 `measureMargin` 挂在**无依赖**的 layout effect 上、每次渲染都执行——两次 `getBoundingClientRect` + 一次 `scrollTop` 读取。流式期间每帧渲染 → 每秒上百次**强制同步布局**（实测 RecalcStyle/Layout ~200 次/秒，归因占全部布局读的 68%），主线程被原生布局工作饱和（点击迟滞、minimap 卡顿，而动画走合成器不受影响），原生分配高水位持续上涨（GC 后 JS 堆全程 ≤195MB，泄漏全在 Blink 原生侧；轮次结束会大幅释放，长审查连续多轮才堆到 GB 级）。已观察 host/scroller/parent 的 ResizeObserver 本就能捕捉上方兄弟高度变化，改为挂载/行集变化时测定初值即可。修复后强制布局读归零，流式期间 RecalcStyle/Layout 降至每帧一次自然重排（~107 次/秒）。
- **输入框上方的 todolist / 本轮 diff 胶囊恢复**：streaming-ver1 重写时 `TaskProgressDock` 被掏成空壳（`return null`）、todo 状态机与 `sessionTaskProgress` 选择器从 `toolMeta.ts` 整体移除，App 侧还挂着空组件与 TODO 注释。本轮自 4788173 恢复三者：todo 状态机（阶段分组/状态映射/HUD 可见性）、会话任务进度选择器（最新 todo 快照 + 当前轮文件改动，新用户消息开轮时上一轮文件落下）、完整 Dock 组件与 App 接线；预览 fixture 通路不变。
- **「回到最新」按钮固定到输入框右上角**：原挂在对话流内 `position: sticky` 跟内容走。现由 `ConversationPane` 把 `{visible, jumpToLatest}` 以 render-phase slot 外发给宿主，按钮挂在 `.composer`（新增 `position: relative`）内、绝对定位于输入框上缘之外右上角（`top:-44px; right:10px`），恒定不随对话滚动。脱离判定同步收紧：**读者主动脱离尾部即亮按钮**（原先只在「脱离后又有新内容到达」时才弹，纯滚动回看时没有回去的入口）。点击回底、贴底或滚回 72px 内自动消失的行为不变。
- **工具链/工具卡折叠时序**：流式轮次中，两次模型请求之间的间隔（正文已落盘、turn 仍 open）会把尾链收起、尾卡折叠——观感是「正文一出工具链就急着收走」。`liveTail` 判定从 `status === "streaming"` 放宽为 `streaming || turnOpen`：轮次未结束前尾链保持自动展开、尾卡保持跟随，下一条正文出现才折叠上一条工具链，轮次结束才折叠整链。
